...
import { ensureAdminSeed, signup, loginWithGoogle, getSession } from '@/lib/auth'
...
export default function SignupPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    const result = await signup({ email, name, password, phone, role: 'customer' })
    setSubmitting(false)
    if (!result.ok) {
  ...
  const handleGoogle = async () => {
    ...
    router.replace(result.session?.role === 'admin' ? '/admin' : '/dashboard')
  }
