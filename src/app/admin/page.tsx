...
const customerAnalytics = [
  ...
  { month: "Apr", new: 135, returning: 390 },
  { month: "May", new: 180, returning: 450 },
  { month: "Jun", new: 210, returning: 510 },
  { month: "Jul", new: 195, returning: 480 },
  { month: "Aug", new: 230, returning: 540 },
  { month: "Sep", new: 250, returning: 580 },
  { month: "Oct", new: 240, returning: 560 },
  { month: "Nov", new: 270, returning: 620 },
  { month: "Dec", new: 300, returning: 680 },
];
      ...
      {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={0} y={y - 4} fontSize={10} fill="#888" textAnchor="start">
              {val}k
            </text>
          </g>
        );
      ...
      {monthlyRevenue.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            />
            <text x={x + barW / 2} y={chartH + 16} fontSize={10} fill="#666" textAnchor="middle">
              {d.month}
            </text>
          </g>
        );
...
function DonutChart() {
  ...
  const segments = orderCategories.reduce<
    { color: string; dashLen: number; offset: number }[]
  >((acc, cat, _i, arr) => {
  ...
  return (
    <div className="flex items-center gap-6">
      <svg viewBox="0 0 240 240" className="w-48 h-48 shrink-0">
        ...
        {/* Center text */}
        <text x={cx} y={cy - 8} textAnchor="middle" fontSize={22} fontWeight="bold" fill={colors.deep}>
          {total}%
        </text>
...
function AreaChart() {
  ...
  const toAreaD = (pts: { x: number; y: number }[]) =>
    `${toPathD(pts)} L${pts[pts.length - 1].x},${padY + plotH} L${pts[0].x},${padY + plotH} Z`;
  ...
  return (
    <svg viewBox={`0 0 ${chartW} ${chartH + 30}`} className="w-full h-auto">
      ...
      {[0, 0.5, 1].map((pct, i) => {
        ...
        return (
          <line key={i} x1={padX} y1={y} x2={chartW - padX} y2={y} stroke="#e5e0d5" strokeWidth={0.5} strokeDasharray="4,4" />
        );
      })}
      {/* Returning area */}
      ...
      {returningPoints.map((p, i) => (
        <circle key={`r${i}`} cx={p.x} cy={p.y} r={3} fill={colors.maroon} />
      ))}
      {/* Dots - new */}
      {newPoints.map((p, i) => (
        <circle key={`n${i}`} cx={p.x} cy={p.y} r={3} fill={colors.saffron} />
      ))}
      {/* Labels */}
      {customerAnalytics.map((d, i) => {
        const x = padX + (i / (customerAnalytics.length - 1)) * plotW;
        return (
          <text key={i} x={x} y={chartH + 16} fontSize={9} fill="#888" textAnchor="middle">
            {d.month}
          </text>
        );
      })}
  ...
  return (
    <div className="flex min-h-screen" style={{ backgroundColor: colors.warmCream }}>
      ...
      <style>{`
        ...
        .card-enter { animation: fadeIn 0.4s ease-out both; }
        ...
        .gold-border {
          border-image: linear-gradient(135deg, ${colors.templeGold}, ${colors.saffron}, ${colors.templeGold}) 1;
        }
        .temple-header-bg {
          background: linear-gradient(135deg, ${colors.maroon}, ${colors.deep});
        }
        .stat-gradient-revenue {
          background: linear-gradient(135deg, ${colors.saffron}15, ${colors.templeGold}10);
        }
        .stat-gradient-orders {
          background: linear-gradient(135deg, ${colors.maroon}12, ${colors.saffron}08);
        }
        .stat-gradient-customers {
          background: linear-gradient(135deg, ${colors.templeGold}12, ${colors.maroon}08);
        }
        .stat-gradient-conversion {
          background: linear-gradient(135deg, ${colors.deep}10, ${colors.maroon}08);
        }
      ...
      <aside
        ...
        } shrink-0 temple-header-bg text-white flex flex-col transition-all duration-300 relative z-30 sidebar-enter`}
      >
        {/* Temple ornament top */}
        <div
          className="px-5 pt-5 pb-3"
          style={{ borderBottom: `1px solid ${colors.templeGold}40` }}
        >
          <div className="flex items-center gap-3">
            <div
              className="w-10 h-10 rounded-lg flex items-center justify-center gold-glow"
              style={{ backgroundColor: colors.templeGold, color: colors.deep }}
            >
              <Landmark className="w-6 h-6" />
            </div>
            <div>
              <h1 className="font-bold text-lg leading-tight" style={{ color: colors.templeGold }}>
                Shri Fragrance
              </h1>
          ...
          {/* Decorative line */}
          <div className="mt-3 h-px w-full" style={{ background: `linear-gradient(90deg, transparent, ${colors.templeGold}60, transparent)` }} />
        </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                style={
                  isActive
                    ? { background: `linear-gradient(135deg, ${colors.saffron}, ${colors.maroon})` }
                    : undefined
                }
              >
                ...
                {isActive && (
                  <div className="ml-auto w-1.5 h-1.5 rounded-full" style={{ backgroundColor: colors.templeGold }} />
                )}
              </button>
            );
          })}
        ...
        {/* Sidebar footer */}
        <div className="px-4 py-4" style={{ borderTop: `1px solid ${colors.templeGold}30` }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: colors.templeGold, color: colors.deep }}>
              SA
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium truncate">Shri Admin</p>
              <p className="text-xs opacity-50 truncate">admin@shrifragrance.com</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* ── Top Bar ──────────────────────────────── */}
        <header
          className="sticky top-0 z-20 flex items-center gap-3 px-4 md:px-6 py-3 border-b"
        ...
        >
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              ...
              style={{ borderColor: `${colors.templeGold}30` }}
            />
          </div>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ backgroundColor: colors.maroon, color: colors.warmCream }}>
                  SA
                </div>
              ...
              {profileOpen && (
                <>
                  ...
                  <div
                    className="absolute right-0 top-11 w-48 bg-white rounded-lg shadow-xl border py-1 z-50"
                    style={{ borderColor: `${colors.templeGold}25` }}
                  >
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              ...
              style={{ backgroundColor: `${colors.templeGold}15`, color: colors.maroon }}
            >
              <Sparkles className="w-3.5 h-3.5" />
              Navratri Season Active
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {statsData.map((stat, i) => {
              ...
              return (
                <Card
                  key={stat.title}
                  className={`card-enter ${delayClass} border-0 shadow-md hover:shadow-lg transition-shadow duration-300`}
                  style={{ borderColor: `${colors.templeGold}20`, borderTop: `3px solid ${iconBgs[i]}` }}
                >
                  <CardContent className="p-5">
                    <div className={`rounded-xl p-4 ${gradients[i]}`}>
                      <div className="flex items-center justify-between mb-3">
                        <div
                          className="w-10 h-10 rounded-lg flex items-center justify-center"
                          style={{ backgroundColor: iconBgs[i], color: colors.warmCream }}
                        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {/* Revenue Chart */}
            <Card
              className="lg:col-span-2 border shadow-md card-enter"
              style={{ borderColor: `${colors.templeGold}20` }}
            >
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base" style={{ color: colors.deep }}>
                    Revenue Overview
                  </CardTitle>
                  <div className="flex items-center gap-1 text-xs text-gray-400">
                    <TrendingUp className="w-3.5 h-3.5" style={{ color: colors.saffron }} />
                    +18.2% vs last year
                  </div>
                </div>
              </CardHeader>
              ...
              <CardContent className="px-0">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr
                        className="border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                        style={{ borderColor: `${colors.templeGold}15` }}
                      >
                    ...
                    <tbody>
                      {recentOrders.map((order, i) => (
                        <tr
                          ...
                          style={{ borderColor: `${colors.templeGold}10` }}
                        >
                          <td className="px-6 py-3 font-medium" style={{ color: colors.deep }}>
                            {order.id}
                          </td>
                ...
                {topProducts.map((product, i) => (
                  <div key={i} className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs font-semibold shrink-0" style={{ color: colors.saffron }}>
                      ...
                      <div className="flex-1 h-2 rounded-full bg-gray-100 overflow-hidden">
                        <div
                          ...
                          style={{
                            ...
                            background: `linear-gradient(90deg, ${colors.saffron}, ${colors.templeGold})`,
                          }}
                        />
                      </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-4 text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className="w-3 h-0.5 rounded-full" style={{ backgroundColor: colors.maroon }} />
                    Returning
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-3 h-0.5 rounded-full" style={{ backgroundColor: colors.saffron }} />
                    New
                  </span>
                </div>
              </div>
          ...
          <div className="text-center py-4">
            <div
              className="h-px w-40 mx-auto mb-3"
              style={{ background: `linear-gradient(90deg, transparent, ${colors.templeGold}40, transparent)` }}
            />
            <p className="text-xs text-gray-400">
              Shri Fragrance Admin &middot; Crafted with devotion
            </p>
          </div>
        </main>
      </div>
